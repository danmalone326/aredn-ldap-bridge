...
1. **Fail Open for LDAP Clients**  
   Phones are inconsistent. If behavior is ambiguous, prefer accepting and responding rather than rejecting.

2. **No Background Polling**  
   Cache refresh MUST be lazy and demand-driven only.

3. **AREDN Is the Source of Truth**  
...
6. **Deterministic, Readable Code**  
   Prefer clarity and explicit behavior over clever abstractions.
...
- **Language:** Python 3.x
- **Runtime:** Linux (Debian/Ubuntu/Alpine)
- **Network:** Plain TCP (no TLS)
- **State:** In-memory only

Agents SHOULD prefer:
- Standard library modules
- Well-maintained, lightweight dependencies

Agents SHOULD avoid:
- Heavy frameworks
- ORMs
- Full OpenLDAP deployments
...
Agents MUST assume development occurs inside a **Python virtual environment** named `venv`.

- All Python commands SHOULD be run with the virtual environment activated
- Dependencies MUST be installed into `venv`, not system Python
- Documentation and instructions SHOULD reference:
...
## LDAP-Specific Guidance

Agents implementing LDAP functionality MUST:

- Accept anonymous bind
- Accept simple bind with any DN/password
- Ignore bind credentials entirely
- Ignore base DN and schema strictness
- Support LDAP v3 search with:
  - equality filters
  - substring filters
  - AND / OR combinations
  - presence filters
...
- Require StartTLS / LDAPS

---

## Caching Rules

Agents MUST implement caching exactly as specified:

- TTL-based, in-memory cache
- Default TTL: 60 seconds
- Refresh only on LDAP search
- No refresh when idle
- Single refresh in-flight at a time
- Serve last-known-good cache on upstream failure
...
- Prefer partial results over hard failure
- Log errors clearly and with context
- Do not crash on malformed LDAP requests
- Do not propagate upstream HTTP errors directly to LDAP clients
...
- Cache refresh attempts are logged
- Upstream node failures are logged
- LDAP bind and search activity is logged (may be summarized)
...
- Provide unit tests for:
  ...
  - matching logic
  - cache behavior
- Provide integration tests using:
  - mocked AREDN sysinfo responses
  - `ldapsearch`

Tests MUST NOT:
- Require live AREDN nodes
- Depend on specific phone hardware
...
- It meets the design spec requirements
- It works with Cisco SPA-514G Corporate Directory
- It does not introduce background polling
- It includes appropriate logging
- It includes tests or clear validation steps

---

## Communication Style for Agents

When generating plans, code, or explanations:

- Be explicit
- Avoid hidden assumptions
- Prefer bullet points and stepwise reasoning
...
This project prioritizes **reliability, predictability, and interoperability** on a constrained mesh network.
